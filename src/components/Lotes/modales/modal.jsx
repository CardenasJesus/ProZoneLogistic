                ...
                <div className='w-full flex justify-center items-center content-center'>
                    <>
                        ...
                        <dt className="px-6 py-4 font-medium  whitespace-nowrap  text-center">
                                        {selectedLotes.nombre_lote} 
                                        </dt>
                                        <dt className="px-6 py-4 text-center">
                                            {selectedLotes.descripcion_lote}
                                        </dt>
                                        <dt className="px-6 py-4 text-center">
                                           Stock: {selectedLotes.existencia_lote}
                                        </dt>
                                        <dt className="px-6 py-4 text-center">
                                           Peso: {selectedLotes.peso} kg
                                        </dt>
                                        <dt className="px-6 py-4 text-center">
                                           Precio: ${selectedLotes.precio_lote} 
                                        </dt>
                                        <dt className="px-6 py-4 text-center">
                                           Fecha de ingreso: {selectedLotes.fecha_entrada} 
                                        </dt>
                                        <dt className="px-6 py-4 text-center">
                                           Fecha de Vencimiento: {selectedLotes.vencimiento_producto} 
                                        </dt>
                                    <div className='grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-2'>
                                        <nav className='flex justify-center items-center text-center'>
                                            <div>
                                            <h1 className='font-bold text-sm text-center'>Almacen</h1>
                                            <p><span>ID</span> {selectedLotes.almacen?.id}</p>
                                            <p>{selectedLotes.almacen?.nombre}</p>
                                            </div>
                                        </nav>
                                        <nav className='flex justify-center items-center text-center'>
                                            <div>
                                            <h1 className='font-bold text-sm text-center'>Proveedor</h1>
                                            <p><span>ID</span> {selectedLotes.proveedor?.id}</p>
                                            <p>{selectedLotes.proveedor?.nombre}</p>
                                            </div>
                                        </nav>
                                        <nav className='flex justify-center items-center text-center'>
                                            <div>
                                            <h1 className='font-bold text-sm text-center'>Producto</h1>
                                            <p><span>ID</span> {selectedLotes.producto?.id}</p>
                                            <p>{selectedLotes.producto?.nombre_producto}</p>
                                            </div>
                                        </nav>
                                    </div>
                        </div>
...
const UpdateModalLotes = ({open, handleClose, selectedLotes}) => {
    ...
    const handlePatterns = (e) => {
      ...
      useEffect(() => {
        if (selectedLotes) {
          setFormsData({
            nombre: selectedLotes.nombre || '',
            apellido: selectedLotes.apellido || '',
            telefono: selectedLotes.telefono || '',
            direccion: selectedLotes.direccion || '',
          });
          setNombreProv(selectedLotes.nombre || '');
          setApellidoprov(selectedLotes.apellido || '');
          setTelefono(selectedLotes.telefono || '');
        }
      }, [selectedLotes]);
      ...
      const sendUpdUser = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch(`http://127.0.0.1:8000/v1/api/proveedores/update/${selectedLotes.id}/`, {
                method: 'PUT',
                headers: {
...
const CreateModalLotes = ({ openLot, handleCloseLotes, getProveedores}) => {
    const [nombre, setNombreProv] = useState('');
    const [apellido, setApellidoProv] = useState('');
    const [telefono, setTelefono] = useState('');
    const validateValues = (value,pattern) => {
