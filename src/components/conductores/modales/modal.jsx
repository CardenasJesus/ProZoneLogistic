      ...
      useEffect(() => {
        if (selectedConductor) {
          setFormsData({
            ...
            telefono: selectedConductor.telefono || '',
...
const AsignarVehiculoModal = ({open, handleCloseConductorUpd, selectedConductor, vehiculos}) => {
    const [formsData, setFormsData] = useState({
        conductor_id: '',
        vehiculo_id: '',
    ...
    const AsignVehicle = async (e) => {
        ...
        .then(response => {
            ...
            response.json()
                ...
                (error => {
                    throw error || 'Error Asigning vehicle'
                })
            }
        })
        .then(data => {
            console.log(data);
            toast.success('Asigning vehicle successfully')
        })
        .catch(error => {
            console.log('error', error);
            toast.error('Error Asigning vehicle')
        })  
    }
                ...
                <Box sx={style}>
                    ...
                    <nav className='w-full p-4 grid grid-cols-1'>
                        ...
                        <form onSubmit={handleSubmit} className="w-full p-4">
                            <div className='w-full grid lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1 gap-4 mb-4 '>
                                            <div>
                                                <label htmlFor="conductorID" className="block mb-4 text-sm font-medium text-gray-900 dark:text-white">Conductor: </label>
                                            <select id="conductorID" name="conductor_id"
                                            ...
                                            {Array.isArray(selectedConductor) && selectedConductor.map((conductor) => (
                                            ...
                                            <div>
                                                <label htmlFor="vehiculo_id" className="block mb-4 text-sm font-medium text-gray-900 dark:text-white">Vehiculos: </label>
                                            <select id="vehiculo_id" name="vehiculo_id"
                                            onChange={handleInputChange} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                            <option disabled selected>Seleccione un Vehiculo</option>
                            ...
                            <div>
                            <button
                                    type="submit" onClick={handleCloseConductorUpd}
                                    className="bg-white text-center w-48 rounded-2xl h-14 relative font-sans text-black text-base font-semibold group"
                                    >
                                    <div
                                        className="bg-blue-400 rounded-xl h-12 w-1/4 flex items-center justify-center absolute left-1 top-[4px] group-hover:w-[184px] z-10 duration-500"
...
export {UpdateModalConductores, CreateModalConductores,AsignarVehiculoModal};